...
heroVideos.forEach(video=>{
  ...
  video.setAttribute('muted','');video.setAttribute('playsinline','');video.setAttribute('webkit-playsinline','');video.preload='auto';
  const source=video.querySelector('source');
  if(source&&source.src&&!source.src.includes('v=hero2')){const url=new URL(source.src,window.location.href);url.searchParams.set('v','hero2');source.src=url.href;video.load();}
  const tryPlay=()=>{const p=video.play();if(p&&typeof p.catch==='function')p.catch(()=>{});};
  if(video.readyState>=2)tryPlay();else video.addEventListener('canplay',tryPlay,{once:true});
  window.addEventListener('pageshow',tryPlay);document.addEventListener('visibilitychange',()=>{if(!document.hidden)tryPlay();});
});

const heroLead=document.querySelector('.hero .lead');if(heroLead)heroLead.textContent='Från skönt mingel till allsång och fullt ös. Livemusik för fest, afterski, after beach, företag, pub och restaurang.';
...
document.querySelectorAll('.testimonial').forEach(testimonial=>{const name=testimonial.querySelector('figcaption strong')?.textContent.trim();const quote=testimonial.querySelector('blockquote');if(quote&&name==='Eddy Estigarribia')quote.textContent='”Vi har haft ett långt och mycket gott samarbete med Henke. Han är en fantastisk sångare och entertainer som levererar varje gång. Vi har samarbetat i över ett decennium och hoppas ha honom hos oss länge till.”';if(name==='Markus Pettersson'){if(quote)quote.textContent='”Henkes spelningar hos oss på Fårö har varit episka. Varje år ser vi fram emot att han ska komma tillbaka till oss, något som blivit en tradition sedan flera år tillbaka.”';const logo=testimonial.querySelector('.testimonial-logo');if(logo)logo.src='images/ref/Majkens-v2_vit_plain_2-495x374-2.png';}});
...
if(bookingForm){
  bookingForm.setAttribute('action','https://formspree.io/f/xnpalpgg');bookingForm.setAttribute('method','POST');
  ...
  const tech=bookingForm.querySelector('#tech');
  if(tech){const field=tech.closest('.form-field');if(field)field.remove();}

  const message=bookingForm.querySelector('#message');
  if(message){message.required=false;message.removeAttribute('required');message.placeholder='Tider, upplägg, publik, önskemål eller annat som är bra för mig att känna till.';const label=bookingForm.querySelector('label[for="message"]');if(label)label.textContent='Övrig information';}

  bookingForm.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(field=>field.setAttribute('data-fs-field',''));
  const submitButton=bookingForm.querySelector('.form-submit');if(submitButton){submitButton.setAttribute('data-fs-submit-btn','');submitButton.removeAttribute('aria-disabled');}
  ...
  const status=document.createElement('div');status.className='form-status';status.setAttribute('aria-live','polite');status.innerHTML='<div class="form-success" data-fs-success hidden>Tack! Din bokningsförfrågan är skickad. Jag återkommer så snart jag kan.</div><div class="form-error" data-fs-error hidden>Något gick fel. Försök igen eller kontakta mig direkt.</div>';bookingForm.insertBefore(status,submitButton||null);
  const formStatusStyle=document.createElement('style');formStatusStyle.textContent='.form-status{grid-column:1/-1}.form-success,.form-error{margin:0 0 18px;padding:14px 16px;border-radius:10px;font-size:.95rem;line-height:1.45}.form-success{background:#eef7ee;border:1px solid #bdd8bd;color:#254d25}.form-error{background:#fff1f0;border:1px solid #e3b8b4;color:#7a2720}.form-success[hidden],.form-error[hidden]{display:none!important}.form-submit:disabled{opacity:.65;cursor:wait;transform:none;box-shadow:none}';document.head.appendChild(formStatusStyle);
  window.formspree=window.formspree||function(){(window.formspree.q=window.formspree.q||[]).push(arguments);};window.formspree('initForm',{formElement:'#booking-form',formId:'xnpalpgg'});const formspreeScript=document.createElement('script');formspreeScript.src='https://unpkg.com/@formspree/ajax@1';formspreeScript.defer=true;document.head.appendChild(formspreeScript);
}
